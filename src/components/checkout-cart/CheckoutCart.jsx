        ...
        if (cartData) {
            try {
                ...
                const itemsWithDiscount = parsedItems.map(item => {
                    ...
                    return {
                        ...
                        product_id: item.product_id || item.id // Use product_id from cart
                    };
                });
            ...
            try {
                ...
                const itemWithDiscount = {
                    ...
                    // Ensure design properties are preserved
                    is_design: singleItem.is_design || false,
                    front_image: singleItem.front_image || '',
                    back_image: singleItem.back_image || '',
                    design_id: singleItem.design_id || null,
                    design_name: singleItem.design_name || '',
                    color_name: singleItem.color_name || singleItem.color || '',
                    hex_code: singleItem.hex_code || '',
                    color_id: singleItem.color_id || null,
                    product_id: singleItem.product_id || singleItem.id
                };
        ...
        try {
            const orderData = {
                ...
                items: items.map((item) => {
                    const baseItem = {
                        product_id: Number(item.product_id), // Use product_id instead of id
                                                ...
                                                {item.is_design && item.hex_code && (
                                                    <div className="flex items-center gap-2 text-sm">
                                                        ...
                                                        <div 
                                                            ...
                                                            title={item.hex_code}
                                                        />
                                                        <span>{item.color_name || item.color}</span>
                                                    </div>
                                                )}
