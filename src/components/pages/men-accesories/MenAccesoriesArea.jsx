  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const cached = !isHardReload && sessionStorage.getItem("men_accessories_products");
        ...
        const accessories = (response.data?.data || []).filter((product) =>
          matchesProductCategory(product, "men", ["accessories", "accessory"])
        );

        setProducts(accessories);
        sessionStorage.setItem("men_accessories_products", JSON.stringify(accessories));
      } catch (error) {
        console.error("Failed to load men's accessories", error);
      } finally {
        setLoading(false);
      }
    };
...
export default MenAccesoriesArea;
