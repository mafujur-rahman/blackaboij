...
const MenOutwearArea = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (!isHardReload) {
        const cached = sessionStorage.getItem("men_outwear_products_v2");
        if (cached) {
      ...
      if (res.data?.success) {
        const menOutwears = res.data.data.filter((product) =>
          matchesProductCategory(product, "men", ["outwears", "outerwear"])
        );
